/**
 * infrastructure-warning.js
 *
 * Page logic for the "Warning Infrastructure Equipment" view.
 *
 * Backend provides:
 *   GET  /api/infrastructure-warning
...
function renderTable(rows) {
  ...
  for (const row of rows) {
    ...
    statusTd.innerHTML = `<span class="badge badge--warning">${row.status || "Warning"}</span>`;
...
// Fetches the visible WARNING infrastructure rows from the backend.
async function fetchInfrastructureWarningRows() {
  const res = await fetch("/api/infrastructure-warning", { cache: "no-store" });
...
async function refresh() {
  try {
    const payload = await fetchInfrastructureWarningRows();
