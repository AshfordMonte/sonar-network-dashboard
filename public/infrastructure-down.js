/**
 * infrastructure-down.js
 *
 * Page logic for the "Down Infrastructure Equipment" view.
 *
 * Backend provides:
 *   GET  /api/infrastructure-down
 *   POST /api/suppressions/infrastructure-items/:id
 */

// Looks up a page element by its DOM id.
...
// Caches the DOM elements used throughout the page lifecycle.
const ui = {
  rows: el("rows"),
  empty: el("empty"),
  count: el("count"),
...
// Renders the visible down rows into the table.
function renderTable(rows) {
  ...
  for (const row of rows) {
    ...
    statusTd.innerHTML = `<span class="badge badge--down">${row.status || "Down"}</span>`;
...
// Fetches the visible down infrastructure rows from the backend.
async function fetchInfrastructureDownRows() {
  const res = await fetch("/api/infrastructure-down", { cache: "no-store" });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return await res.json();
}

// Stores the last fetched rows so text filtering stays client-side.
let lastRows = [];
...
async function refresh() {
  try {
    const payload = await fetchInfrastructureDownRows();
...
// Handles suppress button clicks for the current table rows.
async function handleSuppressClick(e) {
...
document.addEventListener("click", handleSuppressClick);
init();
