 ...
 * uninventoried.js
 *
 * Page logic for the "Uninventoried Customers" view.
 *
 * Backend provides:
 *   GET /api/uninventoried-customers
...
function renderTable(customers) {
  ...
  for (const c of customers) {
    ...
    statusTd.innerHTML =
      `<span class="badge badge--neutral">${c.status || "Uninventoried"}</span>`;
...
async function fetchUninventoriedCustomers() {
  const res = await fetch("/api/uninventoried-customers", { cache: "no-store" });
